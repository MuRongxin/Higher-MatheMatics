...
class Infinitesimal(MovingCameraScene):
    ...
    def introduce_concept(self):
        ...
        surrend_part=SurroundingRectangle(
            emphasize_text[1],
            color=_color_1, stroke_width=2, fill_opacity=0)
        ...
        emphasize_title=Text(emphasize_text[1].text).move_to(UL)
        
        # 淡出文字
        self.play(LaggedStartMap(FadeOut,
                VGroup(concept_text,surrend_part,emphasize_text))
        )

        # self.play(Create(emphasize_title))
        self.add(emphasize_title)
